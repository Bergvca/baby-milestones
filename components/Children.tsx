...
async function fetchChilds(
  ...
  const data = await fetchJsonWithAuth<unknown>(
    `${FAMILY_PATH}/${familyID}/children`,
    token,
    signal,
  );
  ...
  const handleAddChild = async () => {
    ...
    if (familyId === null) {
      ...
      try {
        ...
        const createdFamily = await createOrUpdateFamily(
          familyName,
          familyDescription,
          false,
          null,
  ...
  return (
    <View style={screenStyles.menuSection}>
      ...
      {childrenLoading ? (
        ...
        <View>
          {children.map((child) => (
            ...
            >
              <ProfileAvatar size={40} editable={false} isChild={true} childId={child.id} />
      ...
      >
        ...
        <Text style={screenStyles.menuText} disabled={loading || !familyName}>
          Add child
        </Text>
